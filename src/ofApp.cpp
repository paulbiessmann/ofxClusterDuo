...
//        nMax = 30;
    ...
    while(receiver.hasWaitingMessages()){
        ...
        if(m.getAddress() == "/tm/dance"){
            danceSize = ofMap(m.getArgAsFloat(0), 0, 1, 0.1, 10);;
        }
        if(m.getAddress() == "/tm/cam"){
            camDistSpeedUpdate = m.getArgAsFloat(0);
        }
...
void ofApp::updateGrowingLines(){
    ...
    if(midiMessage.control == 20){ // rad 5
       displacementScale  = ofMap(midiMessage.value, 0, 127, 0.0, 20.0);
    }
    if(midiMessage.control == 4){ // fader 5
        displacementScale  = ofMap(midiMessage.value, 0, 127, 0.0, 5.0);
    }
...
void ofApp::drawStringOfPearls(){
    ...
    bool bMountains      = bDrawMountains;  //geht noch nicht
...
//            ofRotate(-45);
    ...
    //                for(int x=0; x<ofGetWidth() * pearlsSize; x+= spacing*pearlsSize){
                        for(int y=0; y<ofGetWidth() * pearlsSize*1.2; y+= spacing*pearlsSize) {
                            float xAxis = yMid + yMid * 0.3f * sin(y*0.01f + ofGetFrameNum() * 0.01f +1) + x; //*audioVolume?
                            float zAxis = yMid + yMid * 0.3f * sin(y*0.01f + ofGetFrameNum() * 0.01f ) + z ;
...
void ofApp::keyPressed(int key){
    ...
    switch(key){
        case '1': //grow lines: Song "One"
            sceneNumber         = 1;
            sceneName           = "Growing Lines - One";
            closeFunction();
            setupGrowingLines();
            bGrowLine           = false;
        ...
        case '2': //DOF cloud: Song "Cube"
            sceneNumber         = 2;
            sceneName           = "DOF Cloud - Cube";
            closeFunction();
            setupDanceCloud();
            bDrawDanceCloud     = true;
            bDrawFocusAssist    = true;
            break;
            
        case '3': //trianglecloud: Song "GHM" ????
            sceneNumber         = 3;
            sceneName           = "Triangle Dance CLoud - GHM";
            closeFunction();
            setupDanceCloud();
            bDrawTriangleCloud  = true;
            break;
        ...
        case '6':   // (Barcode &) 3D Line Gitter: Song "Delay Hack"
            sceneNumber         = 6;
        ...
        case 'z':   // 3D Line Gitter: Song "Delay Hack"
            sceneNumber         = 6;
        ...
        case '8':  // Glitch Rectangles: Song "Gruß an Olivier"
            sceneNumber         = 8;
            sceneName           = "Glitch Rectangles - Gruss an Olivier";
            closeFunction();
            setupGlitchRect();
            bDrawGlitchRect     = true;
            break;

            
        case '9':  // Pearl Field     : Song "Loop Song"
            sceneNumber         = 9;
            sceneName           = "Pearl Field - Loop Song";
            closeFunction();
            setupStringOfPearls();
            bDrawStringOfPearls = true;
